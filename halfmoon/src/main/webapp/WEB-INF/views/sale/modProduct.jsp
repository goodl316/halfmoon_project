...
<div id="regProduct">
	...
	<div class="product"> 
		...
		<div class="div-right1">
			<label for="input_img">
				<img class="img_upload" src="/res/img/img_upload.png"/>
			</label>
			<span id="temp_img_cont">
				<!-- 대표이미지 우선 표시 -->
				<c:forEach items="${mod_img_list}" var="item">
					<c:if test="${item == data.p_img_main}">
						<span class="mod_img_span_cont temp_img_span_cont">
							<img class="img_upload span_sub" style="color:red;"
				...
				<c:forEach items="${mod_img_list}" var="item">
					<c:if test="${item != data.p_img_main}">
						<span class="mod_img_span_cont temp_img_span_cont">
							<img class="img_upload span_sub" src="/res/img/sale/p_${data.i_product}/${item}">
							<div class="delete_mod_img span_sub" value="${item}">×</div>
						</span>
					</c:if>
				</c:forEach>
    ...
    <div class="product">
    	...
    	<div class="div-right2">
    		<input type="text" id="p_title" placeholder="상품 제목을 입력해주세요" value="${data.title}">
    	</div>
    </div>
	<div class="product">
		<div class="div-Text2">
    	...
    	<div>
    		<select id="product_type" onchange="typeChange(this)">
         	   ...
         	   <c:forEach var="item" items="${menuList}">
				   <c:if test="${item.i_product_type == data.i_product_type}">
    		...
    		<select id="loc">
            	<option>
           		...
           		<c:forEach var="item" items="${loc}">
					<c:if test="${item.i_loc == data.i_loc}">
						<option value="${item.i_loc}" selected>
								${item.loc_nm}
						</option>
					</c:if>
	...
	<div class="product">
    	...
    	<div>
    		 <input type="text" id='p_nm' placeholder="제품명을 입력해주세요" value="${data.p_nm}">
    	</div>
	</div>
	<div class="product">
		<div class="div-Text2">
    	...
    	<div class="won">
    		<input type="number" id='p_price' placeholder="숫자만 입력해주세요" value="${data.p_price}">원
    	</div>
	</div>
	<div class="product">
		<div class="div-Text7">
	...
	<div class="product">
    	...
    	<div class="explanation">
    		<textarea id="p_tag" placeholder="연관태그를 입력해주세요.(최대 5개)">${data.tag}</textarea><br>
